...
export default class CreateSchedules extends Component {
  constructor(props) {
    super(props);
      ...
      .then((response) => {
        if (response.data.length > 0) {
          this.setState({
            ...
            professor: response.data[0].professor,
          });
        }
      })
      .catch((error) => {
  ...
  onSubmit(e) {
    ...
    axios
      .post("http://localhost:5000/schedules/add", schedule)
    ...
    return (
      <div>
        ...
        <form onSubmit={this.onSubmit}>
          ...
          <div className="form-group">
            <input type="submit" value="Krijo" className="btn btn-primary" />
          </div>
        </form>
      </div>
    );
